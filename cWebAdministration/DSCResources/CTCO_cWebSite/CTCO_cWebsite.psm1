    ...
    (   
        ...
        elseif ($Website.count -eq 1) # A single Website exists with this name.
        {
            ...
            $CimBindings = foreach ($binding in $bindings)
            {
                ...
                New-CimInstance -ClassName CTCO_cWebBindingInformation -Namespace root/microsoft/Windows/DesiredStateConfiguration -Property @{Port=[System.UInt16]$BindingObject.Port;Protocol=$BindingObject.Protocol;IPAddress=$BindingObject.IPaddress;HostName=$BindingObject.Hostname;CertificateSubjectName=$(Get-CertificateSubjectNameFromThumbprint -CertificateThumbprint $BindingObject.CertificateThumbprint -CertificateStoreName $BindingObject.CertificateStoreName);CertificateStoreName=$BindingObject.CertificateStoreName} -ClientOnly
            }
...
function UpdateBindings
{
    ...
    foreach($binding in $BindingInfo)
    {
        ...
        $CertificateThumbprint = Get-CertificateThumbprintFromSubjectName -CertificateSubjectName $Binding.CimInstanceProperties["CertificateSubjectName"].Value -CertificateStoreName $Binding.CimInstanceProperties["CertificateStoreName"].Value
...
# Get certificate Subjectname from Thumbprint
function Get-CertificateSubjectNameFromThumbprint
{
    [CmdletBinding()]
    [OutputType([System.String])]
    Param
    ...
    try
    {
        if($CertificateThumbprint -ne $null)
        {
            $returnValue=(Get-ChildItem -Path "Cert:\LocalMachine\$CertificateStoreName" | Where-Object {$_.Thumbprint -eq "$CertificateThumbprint"}).Subject
        }
        else
        {
            $returnValue=$null
