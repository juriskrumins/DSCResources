...
function ValidateProperties
{
    ...
    try
    {
        ...
        if($Password)
        {
            ...
            $result = $prnContext.ValidateCredentials($UserName,$Password.GetNetworkCredential().Password,"Negotiate")
            if($result)
            {
    ...
    catch [Microsoft.ActiveDirectory.Management.ADIdentityNotFoundException]
    {
        ...
        if($Apply)
        {
            if( $Ensure -ne "Absent" )
            {
                $params = @{ Name = $UserName; UserPrincipalName = "$($UserName)@$($DomainName)"; Enabled = $true; Credential = $DomainAdministratorCredential }
                if( $Password )
                {
